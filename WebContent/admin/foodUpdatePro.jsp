...
<% String a = request.getParameter("food_id"); %>
...
realFolder = context.getRealPath(saveFolder);  //현재 jsp페이지의 웹 어플리케이션상의 절대 경로를 구한다
...
try
{
 ...
 System.out.println(ioe+"잉?");
}catch(Exception ex){
 System.out.println(ex+"헝?");
}
%>

<jsp:useBean id="food" scope="page" class="shop.ShopBean">
</jsp:useBean>
...
<%
 ...
 String food_content = imageUp.getParameter("food_content");
 ...
 food.setFood_count(Integer.parseInt(food_count));
 food.setFood_image(filename);
 food.setFood_content(food_content);
 food.setReg_date(new Timestamp(System.currentTimeMillis()));
